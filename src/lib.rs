...
impl State {
    ...
    pub fn update(&mut self, mut input: &[u8]) {
        ...
        if self.buflen > 0 {
            ...
            if !input.is_empty() {
                compress_simd(&mut self.h, &self.buf, self.count, 0);
                self.buflen = 0;
            }
        }
        ...
        while input.len() > BLOCKBYTES {
            ...
            compress_simd(&mut self.h, array_ref!(input, 0, BLOCKBYTES), self.count, 0);
    ...
    pub fn _bench_compress_simd(&mut self, block: &[u8; BLOCKBYTES]) {
        ...
        // properly finalized.
        self.count += BLOCKBYTES as u128;
        compress_simd(&mut self.h, block, self.count, 0);
    }
}
